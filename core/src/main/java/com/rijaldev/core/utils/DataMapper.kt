package com.rijaldev.core.utils
...
import com.rijaldev.core.data.source.remote.response.VideoItem
...
object DataMapper {
    ...
    fun mapPagingResponseToPagingDomain(input: PagingData<MovieItem>, movieType: MovieType): PagingData<Movie> =
        input.map {
            Movie(
                it.id,
                it.overview,
    ...
    fun mapVideoResponseToDomain(input: List<VideoItem?>?): List<Video>? =
        input?.map {
            Video(
                it?.name,
                it?.official,
